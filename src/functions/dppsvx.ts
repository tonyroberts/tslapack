...
export function dppsvx(FACT: string,
    UPLO: string,
    N: number,
    NRHS: number,
    AP: ndarray<number>,
    ...
    LDX: number): ndarray<number> {
        ...
        let func = emlapack.cwrap('dppsvx_', null, [
        ...
        func(pFACT, pUPLO, pN, pNRHS, pAP, pAFP, pEQUED, pS, pB, pLDB, pX, pLDX, pRCOND, pFERR, pBERR, pWORK, pIWORK, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dppsvx': " + INFO);
        }
