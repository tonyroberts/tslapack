...
export function dsysvx(FACT: string,
    UPLO: string,
    N: number,
    NRHS: number,
    A: ndarray<number>,
    ...
    LDX: number): ndarray<number> {
        ...
        let func = emlapack.cwrap('dsysvx_', null, [
        ...
        func(pFACT, pUPLO, pN, pNRHS, pA, pLDA, pAF, pLDAF, pIPIV, pB, pLDB, pX, pLDX, pRCOND, pFERR, pBERR, pWORK, pLWORK, pIWORK, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dsysvx': " + INFO);
        }
