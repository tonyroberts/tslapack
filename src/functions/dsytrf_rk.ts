...
export function dsytrf_rk(UPLO: string,
    N: number,
    A: ndarray<number>,
    LDA: number): ndarray<number> {

        let func = emlapack.cwrap('dsytrf_rk_', null, [
        ...
        func(pUPLO, pN, pA, pLDA, pE, pIPIV, pWORK, pLWORK, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dsytrf_rk': " + INFO);
        }
