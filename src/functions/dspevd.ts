...
export function dspevd(JOBZ: string,
    UPLO: string,
    N: number,
    AP: ndarray<number>,
    LDZ: number): ndarray<number> {

        let func = emlapack.cwrap('dspevd_', null, [
        ...
        func(pJOBZ, pUPLO, pN, pAP, pW, pZ, pLDZ, pWORK, pLWORK, pIWORK, pLIWORK, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dspevd': " + INFO);
        }
