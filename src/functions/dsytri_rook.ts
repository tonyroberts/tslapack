...
export function dsytri_rook(UPLO: string,
    N: number,
    A: ndarray<number>,
    LDA: number,
    IPIV: ndarray<number>) {

        let func = emlapack.cwrap('dsytri_rook_', null, [
        ...
        func(pUPLO, pN, pA, pLDA, pIPIV, pWORK, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dsytri_rook': " + INFO);
        }
}