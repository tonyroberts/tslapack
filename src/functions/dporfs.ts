...
export function dporfs(UPLO: string,
    N: number,
    NRHS: number,
    A: ndarray<number>,
    LDA: number,
    ...
    LDX: number): ndarray<number> {
        ...
        let func = emlapack.cwrap('dporfs_', null, [
        ...
        func(pUPLO, pN, pNRHS, pA, pLDA, pAF, pLDAF, pB, pLDB, pX, pLDX, pFERR, pBERR, pWORK, pIWORK, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dporfs': " + INFO);
        }
