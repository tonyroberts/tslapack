...
export function dgelqf(M: number,
    N: number,
    A: ndarray<number>,
    LDA: number): ndarray<number> {

        let func = emlapack.cwrap('dgelqf_', null, [
            'number', // [in] M: INTEGER
            'number', // [in] N: INTEGER
        ...
        func(pM, pN, pA, pLDA, pTAU, pWORK, pLWORK, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dgelqf': " + INFO);
        }
