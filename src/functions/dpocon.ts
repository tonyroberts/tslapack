...
export function dpocon(UPLO: string,
    N: number,
    A: ndarray<number>,
    LDA: number,
    ANORM: number): number {

        let func = emlapack.cwrap('dpocon_', null, [
        ...
        func(pUPLO, pN, pA, pLDA, pANORM, pRCOND, pWORK, pIWORK, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dpocon': " + INFO);
        }
