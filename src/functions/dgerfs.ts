...
export function dgerfs(TRANS: string,
    N: number,
    NRHS: number,
    A: ndarray<number>,
    LDA: number,
    ...
    LDX: number): ndarray<number> {
        ...
        func(pTRANS, pN, pNRHS, pA, pLDA, pAF, pLDAF, pIPIV, pB, pLDB, pX, pLDX, pFERR, pBERR, pWORK, pIWORK, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dgerfs': " + INFO);
        }
