...
export function dlasyf_rk(UPLO: string,
    N: number,
    NB: number,
    A: ndarray<number>,
    LDA: number,
    LDW: number): ndarray<number> {
        ...
        let func = emlapack.cwrap('dlasyf_rk_', null, [
        ...
        func(pUPLO, pN, pNB, pKB, pA, pLDA, pE, pIPIV, pW, pLDW, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dlasyf_rk': " + INFO);
        }
