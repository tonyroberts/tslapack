...
export function dtprfs(UPLO: string,
    TRANS: string,
    DIAG: string,
    N: number,
    NRHS: number,
    ...
    LDX: number): ndarray<number> {
        ...
        let func = emlapack.cwrap('dtprfs_', null, [
        ...
        func(pUPLO, pTRANS, pDIAG, pN, pNRHS, pAP, pB, pLDB, pX, pLDX, pFERR, pBERR, pWORK, pIWORK, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dtprfs': " + INFO);
        }
