...
export function dgehd2(N: number,
    ILO: number,
    IHI: number,
    A: ndarray<number>,
    LDA: number): ndarray<number> {

        let func = emlapack.cwrap('dgehd2_', null, [
            'number', // [in] N: INTEGER
        ...
        func(pN, pILO, pIHI, pA, pLDA, pTAU, pWORK, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dgehd2': " + INFO);
        }
