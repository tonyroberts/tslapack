...
export function dtbtrs(UPLO: string,
    TRANS: string,
    DIAG: string,
    N: number,
    KD: number,
    ...
    LDB: number) {
        ...
        let func = emlapack.cwrap('dtbtrs_', null, [
        ...
        func(pUPLO, pTRANS, pDIAG, pN, pKD, pNRHS, pAB, pLDAB, pB, pLDB, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dtbtrs': " + INFO);
        }
}