...
export function dgeevx(BALANC: string,
    JOBVL: string,
    JOBVR: string,
    SENSE: string,
    N: number,
    ...
    LDVR: number): ndarray<number> {
        ...
        let func = emlapack.cwrap('dgeevx_', null, [
            'number', // [in] BALANC: CHARACTER
            'number', // [in] JOBVL: CHARACTER
            'number', // [in] JOBVR: CHARACTER
            'number', // [in] SENSE: CHARACTER
            'number', // [in] N: INTEGER
            ...
            'number', // [out] ABNRM: DOUBLE PRECISION
            'number', // [out] RCONDE: DOUBLE PRECISION[N]
            'number', // [out] RCONDV: DOUBLE PRECISION[N]
            'number', // [out] WORK: DOUBLE PRECISION[MAX(1,LWORK)]
            'number', // [in] LWORK: INTEGER
            'number', // [out] IWORK: INTEGER[2*N-2]
        ...
        let pBALANC = emlapack._malloc(1);
        let pJOBVL = emlapack._malloc(1);
        let pJOBVR = emlapack._malloc(1);
        let pSENSE = emlapack._malloc(1);
        ...
        let pRCONDE = emlapack._malloc(8 * (N));
        let RCONDE = new Float64Array(emlapack.HEAPF64.buffer, pRCONDE, (N));

        let pRCONDV = emlapack._malloc(8 * (N));
        let RCONDV = new Float64Array(emlapack.HEAPF64.buffer, pRCONDV, (N));

        let pWORK = emlapack._malloc(8 * (Math.max(1,LWORK)));
        let WORK = new Float64Array(emlapack.HEAPF64.buffer, pWORK, (Math.max(1,LWORK)));

        let pIWORK = emlapack._malloc(4 * (2*N-2));
        let IWORK = new Int32Array(emlapack.HEAPI32.buffer, pIWORK, (2*N-2));

        func(pBALANC, pJOBVL, pJOBVR, pSENSE, pN, pA, pLDA, pWR, pWI, pVL, pLDVL, pVR, pLDVR, pILO, pIHI, pSCALE, pABNRM, pRCONDE, pRCONDV, pWORK, pLWORK, pIWORK, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dgeevx': " + INFO);
        }
