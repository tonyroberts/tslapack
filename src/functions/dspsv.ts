...
export function dspsv(UPLO: string,
    N: number,
    NRHS: number,
    AP: ndarray<number>,
    B: ndarray<number>,
    LDB: number): ndarray<number> {
        ...
        let func = emlapack.cwrap('dspsv_', null, [
        ...
        func(pUPLO, pN, pNRHS, pAP, pIPIV, pB, pLDB, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dspsv': " + INFO);
        }
