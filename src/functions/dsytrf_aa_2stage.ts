...
export function dsytrf_aa_2stage(UPLO: string,
    N: number,
    A: ndarray<number>,
    LDA: number,
    LTB: number): ndarray<number> {

        let func = emlapack.cwrap('dsytrf_aa_2stage_', null, [
        ...
        func(pUPLO, pN, pA, pLDA, pTB, pLTB, pWORK, pLWORK, pIPIV, pIPIV2, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dsytrf_aa_2stage': " + INFO);
        }
