...
export function dsysv_rk(UPLO: string,
    N: number,
    NRHS: number,
    A: ndarray<number>,
    LDA: number,
    ...
    LDB: number): ndarray<number> {
        ...
        let func = emlapack.cwrap('dsysv_rk_', null, [
        ...
        func(pUPLO, pN, pNRHS, pA, pLDA, pE, pIPIV, pB, pLDB, pWORK, pLWORK, pINFO);
        INFO = emlapack.getValue(pINFO, 'i32');
        if (INFO != 0) {
            throw new Error("Error calling 'dsysv_rk': " + INFO);
        }
